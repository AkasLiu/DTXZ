<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<title>回答</title>
		
		<!--图标-->
		<link rel="shortcut icon " type="images/x-icon" href="${pageContext.request.contextPath}/favicon.ico">
		
		<!--链入基本初始化-->
		<link rel="stylesheet" href="${pageContext.request.contextPath }/public/style/base.css">
		
		<!--链入前台公共样式-->
		<link rel="stylesheet" href="${pageContext.request.contextPath }/public/style/common.css">
		
		<!--提问中的样式 -->
		<link rel="stylesheet" href="${pageContext.request.contextPath }/public/style/essay_answer.css">
		
		<link rel="stylesheet" href="${pageContext.request.contextPath }/public/style/essay_myessay.css" />
		<script type="text/javascript" src="${pageContext.request.contextPath }/public/js/jquery.js" ></script>
		<script type="application/javascript" src="${pageContext.request.contextPath}/public/js/common.js"></script>

	</head>
	<body>
	
	<jsp:include page="${pageContext.request.contextPath}/WEB-INF/view/common/header.jsp"></jsp:include>

	<!--切换标题栏-->
	<script>
        changeNav(1);
	</script>

	<!----------------------------主体------------------------------------>
	<div class="container">
			
		<!-----------------------主体左边------------------------>
		<div class="fl">
			<div class="breadcrumb">
				<a href="essay_question.html">回答</a>
				<span> > </span>
				<span>无序标签不生效问题修复方法</span>
			</div>
			<div class="answer">
				<!--显示用户信息-->
				<div class="question_head">
					<h1>无序标签不生效问题修复方法</h1>
					<div class="question_author">
						<a href="#"><img src="images/QQ-paytitle.png"/></a>
						<div class="detail">
							<span class="nickName"><a href="#">牛妹</a></span>
						</div>
						<div class="foot">
							<span class="foot_left">编辑于 2017-11-16 18:27:51</span>
							<span class="foot_right">回答6&nbsp;&nbsp;</span>
						</div>	
					</div>
				</div>
				<!--问题具体内容-->
				<div class="question_content">
					<p>我们明年的春招是分两个年级的，每个年级的形式和情况都不太一样，所以我们按照年级分了两个通道，大家可以根据自己的情况去查看哦~ </p>
					<p>我们明年的春招是分两个年级的，每个年级的形式和情况都不太一样，所以我们按照年级分了两个通道，大家可以根据自己的情况去查看哦~ </p>
					<p>我们明年的春招是分两个年级的，每个年级的形式和情况都不太一样，所以我们按照年级分了两个通道，大家可以根据自己的情况去查看哦~ </p>
					<p>我们明年的春招是分两个年级的，每个年级的形式和情况都不太一样，所以我们按照年级分了两个通道，大家可以根据自己的情况去查看哦~ </p>
					<p>我们明年的春招是分两个年级的，每个年级的形式和情况都不太一样，所以我们按照年级分了两个通道，大家可以根据自己的情况去查看哦~ </p>
					<p>TO：官方  </p>
					<p>建议如下改动，谢谢！global.css文件的ul没有定义前缀，导致全局ul全部隐藏无序标签</p>
				</div>
				<div class="question_menu">
					<a href="#">
						<span class="next">无序标签不生效问题修复方法&nbsp;&nbsp;&nbsp;</span>
						<span class="next nextq">下一个问题&nbsp;&nbsp;&nbsp;</span>	
						<i></i>
					</a>
				</div>
			</div>
			
			<!--用户回答-->
			<div class="answer_list">
				<div class="answer_list_head">
					<h1>15条回答</h1>
					<a href="#respondid">回答</a>
				</div>
				<div class="answer_list_body">
					<ul>
						<li>
							<div class="answer_content">
								<a href="#"><img src="images/QQ-paytitle.png"/></a>
								<div class="answer_detail">
									<p><a href="#">牛妹备胎</a></p>
									<div class="answer_dricp">
										支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！
										支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！	
									</div>
									<div class="answer_action">
										<span>发表于 2017-11-16 23:54:21</span>
										<a href="javascript:void(0)" class="a_reply">评论(3)</a>
										<a href="javascript:void(0)">赞(0)</a>
									</div>
									<div class="reply_box">
										<div class="reply_list">
											<ul>
												<li>
													<div class="reply_list_item">
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		aaaaaaaaaaaaaaaaaaaaasddddd
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		sadddddddddddddddddddddddddddddddddsdaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasddddd
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		sadddddddddddddddddddddddddddddddddsdaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasddddd
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
													</div>
												</li>
											</ul>
										</div>
										<div class="reply_editoBox">
											<form action="#" method="post">
												<textarea placeholder="请输入你的观点" style="resize: none; height: 20px; width: 472px; font-size:12px; line-height: 18px;"></textarea>
												<input type="submit" value="回复"/>
											</form>
											
										</div>
									</div>
								</div>
								
							</div>
						</li>
						<li>
							<div class="answer_content">
								<a href="#"><img src="images/QQ-paytitle.png"/></a>
								<div class="answer_detail">
									<p><a href="#">牛妹备胎</a></p>
									<div class="answer_dricp">
										支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！
										支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！	
									</div>
									<div class="answer_action">
										<span>发表于 2017-11-16 23:54:21</span>
										<a href="javascript:void(0)" class="a_reply">评论(3)</a>
										<a href="javascript:void(0)">赞(0)</a>
									</div>
									<div class="reply_box">
										<div class="reply_list">
											<ul>
												<li>
													<div class="reply_list_item">
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		aaaaaaaaaaaaaaaaaaaaaaaaaasddddd
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		aaaaaaaaaaa
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		sadddddddddddddddddddddddddddddddddsdaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasddddd
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
													</div>
												</li>
											</ul>
										</div>
										<div class="reply_editoBox">
											<form action="#" method="post">
												<textarea placeholder="请输入你的观点" style="resize: none; height: 20px; width: 472px; font-size:12px; line-height: 18px;"></textarea>
												<input type="submit" value="回复"/>
											</form>
											
										</div>
									</div>
								</div>
								
							</div>
						</li>
						<li>
							<div class="answer_content">
								<a href="#"><img src="images/QQ-paytitle.png"/></a>
								<div class="answer_detail">
									<p><a href="#">牛妹备胎</a></p>
									<div class="answer_dricp">
										支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！
										支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！	
									</div>
									<div class="answer_action">
										<span>发表于 2017-11-16 23:54:21</span>
										<a href="javascript:void(0)" class="a_reply">评论(3)</a>
										<a href="javascript:void(0)">赞(0)</a>
									</div>
									<div class="reply_box">
										<div class="reply_list">
											<ul>
												<li>
													<div class="reply_list_item">
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		sadddddddddddddddddddddddddddddddddsdaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasddddd
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		sadddddddddddddddddddddddddddddddddsdaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasddddd
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																	aaaaaaaa
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
													</div>
												</li>
											</ul>
										</div>
										<div class="reply_editoBox">
											<form action="#" method="post">
												<textarea placeholder="请输入你的观点" style="resize: none; height: 20px; width: 472px; font-size:12px; line-height: 18px;"></textarea>
												<input type="submit" value="回复"/>
											</form>
											
										</div>
									</div>
								</div>
								
							</div>
						</li>
						<li>
							<div class="answer_content">
								<a href="#"><img src="images/QQ-paytitle.png"/></a>
								<div class="answer_detail">
									<p><a href="#">牛妹备胎</a></p>
									<div class="answer_dricp">
										支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！
										支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！支持牛妹！	
									</div>
									<div class="answer_action">
										<span>发表于 2017-11-16 23:54:21</span>
										<a href="javascript:void(0)" class="a_reply">评论(3)</a>
										<a href="javascript:void(0)">赞(0)</a>
									</div>
									<div class="reply_box">
										<div class="reply_list">
											<ul>
												<li>
													<div class="reply_list_item">
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		sadddddddddddddddddddddddddddddddddsdaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasddddd
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		sadddddddddddddddddddddddddddddddddsdaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasddddd
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
														<div class="reply_list_every">
															<div class="reply_main">
																<div class="reply_preson">
																	<a href="#">ARCUS  : </a>
																</div>
																<div class="reply_content">
																		sadddddddddddddddddddddddddddddddddsdaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaasddddd
																</div>
															</div>
															<div class="reply_info">
																<span >今天 16:26:16</span>					
															</div>
														</div>
													</div>
												</li>
											</ul>
										</div>
										<div class="reply_editoBox">
											<form action="#" method="post">
												<textarea placeholder="请输入你的观点" style="resize: none; height: 20px; width: 472px; font-size:12px; line-height: 18px;"></textarea>
												<input type="submit" value="回复"/>
											</form>
											
										</div>
									</div>
								</div>
								
							</div>
						</li>
					</ul>
				</div>
			</div>
			
			<!--分页-->
			<div class="page">
				<ul class = "pagination-ul">
					<li class = "first-page"><a href = "javascript:void(0)">首页</a></li>
					<li class = "prev-page"><a href = "javascript:void(0)">上一页</a></li>
					<li class = "page-number show"><a href = "javascript:void(0)"><p>1<p></a></li>
					<li class = "page-number"><a href = "javascript:void(0)"><p>2<p></a></li>
					<li class = "next-page"><a href = "javascript:void(0)">下一页</a></li>
					<li class = "last-page"><a href = "javascript:void(0)">末页</a></li>
				</ul>
			</div>
			
			<!--回答-->
			<div class="respond" id="respondid">
				<form action="#">
					<textarea></textarea>
					<button type="submit" class="sub" >回答</button>
				</form>		
			</div>
		</div>

		<!---------------------主体右边------------------------->
		<div class="fr">
		<!--天气-->
		<div class="weather-box">
			<img src="images/weather/qintian-wanshang.png" alt="...">
			<span>周三</span>
			<p>天气晴朗,适合刷题</p>

		</div>

		<div class="info-box">
			<a href="#" class="headimg">
				<img src="images/null-headimg.png" alt="...">
			</a>
			<a href="#" class="nikename">sirius
				<!--<i class="female-tip"></i>-->
				<i class="male-tip"></i>
			</a>

			<div class="profile-box">
				<span>答题:10</span>|
				<span>考试:23</span>|
				<span>排名:16000</span>
			</div>

			<ul class="mess-box">
				<li>
					<p>被采纳</p>
					<p>10</p>
				</li>
				<li>
					<p>获赞</p>
					<p>10</p>
				</li>
				<li>
					<p>正确率</p>
					<p>10%</p>
				</li>
				<li>
					<p>平均排名</p>
					<p>10</p>
				</li>

			</ul>

			<ul class="chengjibang">
				<li><a href="#"><span>09-20</span>衡水二中2017第五次模考第4名</a></li>
				<li><a href="#"><span>09-20</span>衡水二中2017第五次模考第4名</a></li>
				<li><a href="#"><span>09-20</span>衡水二中2017第五次模考第4名</a></li>
				<li><a href="#"><span>09-20</span>衡水二中2017第五次模考第4名</a></li>
			</ul>
		</div>

	</div>
		
	</div>

	

	<!-- 尾部 -->
	<jsp:include page="${pageContext.request.contextPath}/WEB-INF/view/common/footer.jsp"></jsp:include>

	</body>
	
	<script type="text/javascript" src="${pageContext.request.contextPath }/public/js/essay_myessay.js" ></script>
	<script type="text/javascript" src="${pageContext.request.contextPath }/public/js/essay_answer.js" ></script>
	
</html>
